...
#if 0
/* This is specific to CLOVER as we see the interlaced mode bit set spuriously
 * and we'd like to better understand the problem or workaround it correctly */
#include <asm-generic/bug.h>
static inline __u32 DE_BE_RUINT32_MODE_CTL(__u32 sel)
{
	__u32 reg_val = DE_BE_RUINT32(sel, DE_BE_MODE_CTL_OFF);
	WARN((reg_val & (1u<<28)), "r:DEBE has interlaced flag set: 0x%u\n", reg_val);
	WARN((reg_val & (0xff8ffefc)), "r:DEBE has some unusual bits set: 0x%u\n", reg_val);
	return reg_val & 0x0fffffff;
}

static inline void DE_BE_WUINT32_MODE_CTL(__u32 sel, __u32 reg_val)
{
	WARN((reg_val & (1u<<28)), "w:DEBE has interlaced flag set: 0x%u\n", reg_val);
	WARN((reg_val & (0xff8ffefc)), "r:DEBE has some unusual bits set: 0x%u\n", reg_val);
	DE_BE_WUINT32(sel, DE_BE_MODE_CTL_OFF, reg_val & 0xefffffff);
}

#else
static inline __u32 DE_BE_RUINT32_MODE_CTL(__u32 sel)
{
	return DE_BE_RUINT32(sel, DE_BE_MODE_CTL_OFF);
}

static inline void DE_BE_WUINT32_MODE_CTL(__u32 sel, __u32 reg_val)
{
	DE_BE_WUINT32(sel, DE_BE_MODE_CTL_OFF, reg_val);
}
#endif
