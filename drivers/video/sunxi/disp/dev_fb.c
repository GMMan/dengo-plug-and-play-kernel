...
s32 disp_fb_to_var(disp_pixel_format format, struct fb_var_screeninfo *var)
{
	switch(format) {
...
s32 var_to_disp_fb(disp_fb_info *fb, struct fb_var_screeninfo *var, struct fb_fix_screeninfo * fix)
{
...
s32 Fb_Init(void)
{
	...
	if(g_fbi.disp_init.b_init) {
		...
		for(i = 0; i<fb_num; i++)	{
			...
			fb_para.output_height = bsp_disp_get_screen_height_from_output_type(0, DISP_OUTPUT_TYPE_HDMI, g_fbi.disp_init.output_mode[0]);
#endif
