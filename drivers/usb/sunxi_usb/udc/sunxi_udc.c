...
static int g_msc_write_debug = 0;
static int g_msc_read_debug = 0;
	...
	if(is_udc_support_dma()){
		...
		for (i = 0; i < DMA_CHAN_TOTAL; i++) {
			...
			if (dma_irq & tmp) {
				...
				writel(BIT(i), USBC_REG_DMA_INTS(dev->sunxi_udc_io->usb_vbase));
...
static int sunxi_udc_get_frame(struct usb_gadget *_gadget)
{
	...
	return USBC_REG_FRNUM(g_sunxi_udc_io.usb_bsp_hdle);
}
...
static int sunxi_udc_start(struct usb_gadget_driver *driver,
...
static int sunxi_udc_stop(struct usb_gadget_driver *driver)
{
	struct sunxi_udc *udc = the_controller;
...
static int sunxi_udc_remove_device_only(struct platform_device *pdev)
{
...
static int __init sunxi_udc_probe(struct platform_device *pdev)
{
	...
	case USB_PORT_TYPE_DEVICE:
		return sunxi_udc_probe_device_only(pdev);
		//break;
...
static int __exit sunxi_udc_remove(struct platform_device *pdev)
{
	...
	switch(udc_cfg->port_info->port_type) {
	case USB_PORT_TYPE_DEVICE:
		return sunxi_udc_remove_device_only(pdev);
		//break;
	case USB_PORT_TYPE_OTG:
...
static int sunxi_udc_resume(struct platform_device *pdev)
{
	...
	sunxi_udc_bsp_init(0, &g_sunxi_udc_io);
	if(is_udc_enable){
...
static struct platform_driver sunxi_udc_driver = {
	...
	.remove		= __devexit_p(sunxi_udc_remove),
